...
/**
 * @title podETH Arbitrage Sell Contract
 * @dev Strategy: Sell overpriced pod tokens
 * @dev Flow: USDC → pfUSDC → podETH → WETH → USDC
 */
contract podETHArbitrageSell is IFlashLoanSimpleReceiver, Ownable {
    ...
    function _executeArbitrage(uint256 amount) internal {
        // Step 1: USDC → pfUSDC (Vault deposit)
        IERC20Extended(USDC).approve(PF_USDC_VAULT, amount);
        ...
        emit StepCompleted("USDC to pfUSDC", pfUsdcBalance);
        
        // Step 2: pfUSDC → podETH (V2)
        IERC20Extended(PF_USDC_VAULT).approve(V2_ROUTER, pfUsdcBalance);
        ...
        emit StepCompleted("pfUSDC to podETH", podBalance);
        ...
        IPodToken(POD_TOKEN).debond(podBalance, tokens, percentages);
        uint256 wethBalance = IERC20Extended(UNDERLYING_TOKEN).balanceOf(address(this));
        emit StepCompleted("podETH to WETH", wethBalance);
        
        // Step 4: WETH → USDC (V3)
        IERC20Extended(UNDERLYING_TOKEN).approve(V3_ROUTER, wethBalance);
        ...
        emit StepCompleted("WETH to USDC", finalUsdcBalance);
    }
