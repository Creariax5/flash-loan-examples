...
contract pSimmiArbitrageSell is IFlashLoanSimpleReceiver, Ownable {
    ...
    function _executeArbitrageLogic(
        ...
        // Step 1: USDC -> ETH (V3 swap)
        emit Debug("Step 1: USDC -> ETH");
        IUSDC(USDC).approve(V3_ROUTER, amount);
        ...
        // Step 2: ETH -> simmi (V3 swap with 1% fee)
        emit Debug("Step 2: ETH -> simmi");
        ...
        require(simmi_balance > 0, "No SIMMI received from swap");
        emit ArbitrageStep("SIMMI received", simmi_balance);

        // Step 3: simmi -> pSimmi (wrap/bond)
        emit Debug("Step 3: simmi -> pSimmi");
        IERC20(SIMMI).approve(INDEX_UTILS, simmi_balance);
        IIndexUtils(INDEX_UTILS).bond(PSIMMI, SIMMI, simmi_balance, 0);
        uint256 pSimmi_balance = IPSIMMI(PSIMMI).balanceOf(address(this));
        require(pSimmi_balance > 0, "No pSimmi received from bond");
        emit ArbitrageStep("pSimmi received", pSimmi_balance);

        // Step 4: pSimmi -> pfUSDC (V2 swap)
        emit Debug("Step 4: pSimmi -> pfUSDC");
        IPSIMMI(PSIMMI).approve(V2_ROUTER, pSimmi_balance);
        ...
        uint256 pfUSDC_balance = IVault(PF_USDC_VAULT).balanceOf(address(this));
        require(pfUSDC_balance > 0, "No pfUSDC received");
        emit ArbitrageStep("pfUSDC received", pfUSDC_balance);

        // Step 5: pfUSDC -> USDC (withdraw/redeem)
        emit Debug("Step 5: pfUSDC -> USDC");
        IVault(PF_USDC_VAULT).redeem(pfUSDC_balance, address(this), address(this));
